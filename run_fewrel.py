...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, threshold):
    ...
    entity2id = {}
    with open("/data3/zzy_tmp/wikidata_drop/entity2id.txt") as fin:
        fin.readline()
        for line in fin:
...
def main():
    ...
    vecs = []
    vecs.append([0]*100)
    with open("/data3/zzy_tmp/wikidata_drop/entity2vec.vec", 'r') as fin:
        for line in fin:
    ...
    model, _ = BertForSequenceClassification.from_pretrained(args.bert_model, state_dict=model_state_dict, num_labels=num_labels)
    model.to(device)
